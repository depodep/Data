...
$stmt = db()->prepare('SELECT prediction_result_id, dataset_id, run_by_user_id, model_type, target_column, training_rows, testing_rows, accuracy, status, created_at FROM prediction_results WHERE dataset_id = :id ORDER BY created_at DESC');
$stmt->execute(['id' => $datasetId]);
$rows = $stmt->fetchAll();
