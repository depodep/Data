...
$stmt = db()->prepare('SELECT analysis_result_id, dataset_id, run_by_user_id, analysis_mode, status, result_summary, metrics_json, insights_json, started_at, completed_at, created_at FROM analysis_results WHERE dataset_id = :id ORDER BY created_at DESC');
$stmt->execute(['id' => $datasetId]);
$rows = $stmt->fetchAll();
